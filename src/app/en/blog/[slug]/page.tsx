...
export default async function EnglishBlogPostPage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    description: post.description, datePublished: post.publishedAt, dateModified: post.modifiedAt,
    mainEntityOfPage: `${SITE_URL}/en/blog/${post.slug}`,
    author: { "@type": "Person", name: SITE_NAME, url: SITE_URL },
    publisher: { "@type": "Person", name: SITE_NAME, url: SITE_URL }, inLanguage: "en",
  };
  return (
    <>
        ...
        <article className="mx-auto max-w-4xl">
          ...
          <div className="space-y-14 py-14">
            {post.sections.map((section) => <section key={section.title}>
              ...
              <div className="mt-5 space-y-5 text-base font-light leading-8 text-muted-foreground">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
