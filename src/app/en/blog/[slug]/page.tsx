...
export function generateStaticParams() {
  return blogPostsEn.map(({ slug }) => ({ slug }));
}
        ...
        <article className="mx-auto max-w-4xl">
                ...
                {post.modifiedAt && post.modifiedAt !== post.publishedAt && (
                  <>
                    ...
                    <span className="text-xs text-brand-red dark:text-rose-400">
                      Updated: {formatBlogDate(post.modifiedAt, "en")}
                    </span>
                  </>
                )}
                <span aria-hidden="true">•</span>
          ...
          {post.faqs && post.faqs.length > 0 && (
            <div className="my-12 rounded-3xl border border-border/80 bg-card/40 p-6 md:p-8">
              <div className="flex items-center gap-2 text-brand-red dark:text-rose-400">
                ...
                <h3 className="text-xs font-bold uppercase tracking-[0.2em]">Frequently Asked Questions</h3>
              </div>
          ...
          <aside className="rounded-3xl border border-brand-red/20 bg-brand-red/[0.04] p-7 md:p-10">
            ...
            <h2 className="mt-3 font-jakarta text-2xl font-bold text-foreground">
              Turn this topic into a working production system.
            </h2>
            <p className="mt-4 leading-7 text-muted-foreground">
              If you need a battle-tested architecture and enterprise-grade implementation rather than a tutorial, explore the related service and technical cases.
            </p>
            <TrackedLink
          ...
          {/* Related Technical Articles & Prev/Next Navigation (Topic Cluster) */}
          <RelatedPosts
            currentSlug={post.slug}
            currentCategory={post.category}
            allPosts={blogPostsEn}
            locale="en"
          />
        </article>
      </main>
