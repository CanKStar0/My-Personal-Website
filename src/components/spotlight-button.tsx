...
export function SpotlightButton({ children, className = "", href, onClick }: SpotlightButtonProps) {
  ...
  const content = (
    <div
      ...
      onClick={onClick}
      className={`relative overflow-hidden bg-transparent border border-zinc-800 hover:border-brand-red/60 text-zinc-300 rounded-full px-8 py-3 font-medium transition-colors duration-300 group ${className}`}
    >
      {/* Spotlight Effect */}
      <motion.div
