...
export function ServicePage({
  ...
  const labels = isEnglish
    ...
    : {
        ...
        proof: "Proje kanıtını inceleyin",
        realProject: "Gerçek proje",
        tech: "Teknik altyapı",
        related: "İlgili hizmetler",
        closingTitle: "Sisteminiz için doğru çözümü birlikte netleştirelim.",
        closingBody:
          "İhtiyacı, mevcut altyapıyı ve başarı ölçütünü konuşarak uygulanabilir bir teknik yol haritası çıkaralım.",
          ...
          <article className="space-y-16">
                ...
                {section.items && (
                  <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                    {section.items.map((item) => (
                      <li
                        key={item}
                        className="flex gap-3 rounded-xl border border-border/60 bg-card/50 p-4 text-sm text-foreground"
                      >
            ...
            {service.project && (
              <section className="rounded-3xl border border-brand-red/20 bg-brand-red/[0.04] p-7 md:p-10">
                ...
                >
                  {service.project.anchor} <ArrowRight className="h-4 w-4" />
                </TrackedLink>
              </section>
            )}
          ...
          <aside className="space-y-8 lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-border/70 bg-card/50 p-6">
              <div className="flex items-center gap-2 text-sm font-bold text-foreground">
                <Code2 className="h-4 w-4 text-brand-red" /> {labels.tech}
              </div>
              <ul className="mt-5 flex flex-wrap gap-2">
                {service.technologies.map((technology) => (
                  <li
                    key={technology}
                    className="rounded-lg border border-border/60 bg-background px-3 py-1.5 text-xs text-muted-foreground"
                  >
                    {technology}
                  </li>
                ))}
              ...
              <ul className="mt-4 space-y-3">
                {service.relatedServices.map((slug) => {
                  ...
                  return (
                    <li key={slug}>
                      <Link
                        href={localizedServicePath(slug, locale)}
                        className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-brand-red"
                      >
                        {related.shortTitle}
